...
- **候補手は1位だけ**。以前は3位まで並べていたが、「ベストの1つを信用したい。
  いっぱいあると使い方がわからなくなる」ので外した（2026-09-13）。
  MultiPV 3 は同じ思考時間を3本に割くので、1本にしたほうが1位の読みも深くなる。
  `export.py` / `slack_watch.py` の既定を 1 にした。3位まで入っている古いJSONもそのまま開ける
...
- **棋神がONのとき ▶ を押すと、本譜ではなく棋神の読み筋を1手ずつ指していく**
  （一手だけでは意図が読めないことが多いので、12手先まで見られるようにした）。
  矢印は常に「読み筋の次の手」を指す。◀ で戻り、読み筋の頭まで戻ったらそのまま本譜を1手戻す
  （棋神はONのまま。矢印を見ながら本譜を遡れる）。`Esc`（または「戻る」）で棋神OFF＝本譜に戻る。
  右の「読み筋」の手を押すとそこまで一気に進む。
  辿っている間は盤の枠が金色になり、グラフの印と棋譜欄は分岐点のまま動かさない
  （読み筋の途中の評価値は持っていないため）。
  読み筋は棋神ONのときに全手分の局面を先に作っておき（`lineAt`）、送るのは配列の添字だけ
